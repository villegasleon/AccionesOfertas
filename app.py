...
def enviar_correo(mensaje):
    subject = 'Notificación de cambio porcentual'
    body = mensaje

#Debes descargar MIMEmultipart para poder enviar correos
    msg = MIMEMultipart()
    ...
    try:
        ...
        server.quit()
        print(f"Correo electrónico enviado: {mensaje}")
    except Exception as e:
        print(f"Error al enviar correo electrónico: {str(e)}")

# Este bucle monitorea los precios de las acciones
while True:
    for symbol in acciones:
        verificar_y_enviar_notificacion(symbol, token)
    time.sleep(1800)  # 1 minutos para la siguiente verificacion (300 segundos)
    print("Esperando para la siguiente verificación...")
